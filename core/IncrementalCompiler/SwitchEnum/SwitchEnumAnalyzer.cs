...
namespace IncrementalCompiler.SwitchEnum {
    /// <inheritdoc />
    ...
    public class SwitchEnumAnalyzer : DiagnosticAnalyzer
    {
        ...
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(NotExhaustiveSwitchRule, DefaultUnreachableRule);
        ...
        private static void Analyze(SyntaxNodeAnalysisContext context) {
            ...
            if (information.NotExhaustiveSwitch)
            {
                var diagnostic = Diagnostic.Create(
                    NotExhaustiveSwitchRule,
                    switchSyntax.Expression.GetLocation(),
        ...
        {
            var type = ModelExtensions.GetTypeInfo(model, node.Expression, ct).Type;
        ...
        private static ImmutableArray<string> GetUnusedSymbolNames(SemanticModel model, SwitchStatementSyntax node, ITypeSymbol type, CancellationToken ct)
        {
            var symbolsUsed = node
                ...
                .Select(l => ModelExtensions.GetSymbolInfo(model, l.Value, ct).Symbol)
                .ToImmutableHashSet();
            return
                type.GetMembers()
